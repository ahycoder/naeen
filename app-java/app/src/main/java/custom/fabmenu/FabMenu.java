...
public class FabMenu extends ViewGroup implements View.OnClickListener {
    ...
    private static final int DEF_SRC_ICON = R.drawable.ic_menu_toolbar;
     ...
     */
    private OnFabMenuItemClickListener mItemClickListener;
    /**
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        TypedArray a = getContext().obtainStyledAttributes(
                ...
                0, 0);
        try {
            mSrcIconResId = a.getResourceId(R.styleable.FabMenu_sfm_src_icon, DEF_SRC_ICON);
            mSizeMenuButton = a.getInt(R.styleable.FabMenu_sfm_size_menu_button, DEF_SIZE_MENU_BUTTON);
            mCollapseOnItemSelected = a.getBoolean(R.styleable.FabMenu_sfm_collapse_on_item_selected,
                                                   DEF_COLLAPSE_ON_ITEM_SELECTED);
            mDurationExpand = a.getInt(R.styleable.FabMenu_sfm_expand_duration, DEF_EXPAND_DURATION);
            mDurationCollapse = a.getInt(R.styleable.FabMenu_sfm_collapse_duration, DEF_COLLAPSE_DURATION);
            mExpandItemRotationDeg = a.getInt(R.styleable.FabMenu_sfm_expand_item_rotation_degrees,
                                              DEF_EXPAND_ITEM_ROTATION_DEG);
            mExpandMenuBtnRotationDeg = a.getInt(R.styleable.FabMenu_sfm_expand_menu_button_rotation_degrees,
                                                 DEF_MENU_BTN_ROTATION_DEG);
            mSpacingMenuItemsPx = a.getDimensionPixelSize(R.styleable.FabMenu_sfm_spacing_menu_items,
                                                          DensityUtils.dp2px(getContext(), DEF_SPACING_MENU_ITEMS_DP));
            mDelayExpandingMenuItems = a.getInt(R.styleable.FabMenu_sfm_delay_expanding_menu_items,
                                                DEF_DELAY_EXPAND_MENU_ITEM);

            ColorStateList menuButtonColor = a.getColorStateList(R.styleable.FabMenu_sfm_menu_button_color);
            if (menuButtonColor != null) {
                mMenuButton.setBackgroundTintList(menuButtonColor);
            }

            mMenuButton.setRippleColor(
                    a.getColor(R.styleable.FabMenu_sfm_menu_button_ripple_color, mMenuButton.getRippleColor()));
    ...
    public void setOnFabMenuItemClickListener(OnFabMenuItemClickListener listener) {
    ...
    public void onClick(View v) {
        if (mItemClickListener != null) {
            mItemClickListener.onFabMenuItemClick(v);
        }
    ...
    public interface OnFabMenuItemClickListener {
        ...
        void onFabMenuItemClick(View view);
    }
     ...
     */
    public static class FabMenuBehavior extends CoordinatorLayout.Behavior<FabMenu> {
        /**
         * The scroll limit of a {@link AppBarLayout} from which a FabMenu should be hidden.
         */
