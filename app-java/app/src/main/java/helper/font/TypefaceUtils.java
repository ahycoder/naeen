...
public final class TypefaceUtils {
    ...
    private static final Map<Typeface, FontTypefaceSpan> sCachedSpans = new HashMap<Typeface, FontTypefaceSpan>();
     ...
     * @return Return {@link Typeface} or null if the path is invalid.
     */
     ...
     */
    public static FontTypefaceSpan getSpan(final Typeface typeface) {
        if (typeface == null) return null;
        synchronized (sCachedSpans) {
            if (!sCachedSpans.containsKey(typeface)) {
                final FontTypefaceSpan span = new FontTypefaceSpan(typeface);
