...
class FontFactory {
    ...
    protected static int[] getStyleForTextView(TextView view) {
        ...
        if (styleIds[0] == -1) {
            ...
            styleIds[0] = FontConfig.get().getClassStyles().containsKey(view.getClass())
                    ? FontConfig.get().getClassStyles().get(view.getClass())
    ...
    protected static boolean parentIsToolbarV7(View view) {
        return FontUtils.canCheckForV7Toolbar() && view.getParent() != null && (view.getParent() instanceof Toolbar);
    }
    ...
    void onViewCreatedInternal(View view, final Context context, AttributeSet attrs) {
        if (view instanceof TextView) {
            ...
            if (TextUtils.isEmpty(textViewFont)) {
                ...
                if (styleForTextView[1] != -1)
                    textViewFont = FontUtils.pullFontPathFromTheme(context, styleForTextView[0], styleForTextView[1], mAttributeId);
                else
                    textViewFont = FontUtils.pullFontPathFromTheme(context, styleForTextView[0], mAttributeId);
            }
            ...
            FontUtils.applyFontToTextView(context, (TextView) view, FontConfig.get(), textViewFont, deferred);
        }

        // AppCompat API21+ The ActionBar doesn't inflate default Title/SubTitle, we need to scan the
        // Toolbar(Which underlies the ActionBar) for its children.
        if (FontUtils.canCheckForV7Toolbar() && view instanceof Toolbar) {
        ...
        } else if (FontConfig.get().isCustomViewTypefaceSupport() && FontConfig.get().isCustomViewHasTypeface(view)) {
    ...
    private Typeface getDefaultTypeface(Context context, String fontPath) {
        if (TextUtils.isEmpty(fontPath)) {
            fontPath = FontConfig.get().getFontPath();
        }
        if (!TextUtils.isEmpty(fontPath)) {
    ...
    private String resolveFontPath(Context context, AttributeSet attrs) {
        ...
        String textViewFont = FontUtils.pullFontPathFromView(context, attrs, mAttributeId);

        // Try view style attributes
        if (TextUtils.isEmpty(textViewFont)) {
            textViewFont = FontUtils.pullFontPathFromStyle(context, attrs, mAttributeId);
        }

        // Try View TextAppearance
        if (TextUtils.isEmpty(textViewFont)) {
            textViewFont = FontUtils.pullFontPathFromTextAppearance(context, attrs, mAttributeId);
        }
